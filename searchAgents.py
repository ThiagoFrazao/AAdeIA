...
class PositionSearchProblem(search.SearchProblem):
  ...
  def __init__(self, gameState, costFn = lambda x: 1, goal=(1,1), start=None, warn=True, startStatao=None):
    """
    ...
    self.fantasmas = gameState.getGhostPositions()

    # gambi
    if startStatao == None:
      self.startState = gameState.getPacmanPosition()
    else:
      self.startState = startStatao

    if start != None: self.startState = start
    self.goal = goal
    self.costFn = costFn
    if warn and startStatao == None and (gameState.getNumFood() != 1 or not gameState.hasFood(*goal)):
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not self.walls[nextx][nexty] and (nextx, nexty) not in self.fantasmas:
...
class BuscaPacman(SearchAgent):
  ...
  def prob(self, pacman_pos, ghost_pos):
    prob = self.searchType(state, pacman_pos, ghost_pos )
    movimentos = self.searchFunction(prob)

    # retorna uma lista de movimentos
    return movimentos

  # def __init__(self, gameState, costFn = lambda x: 1, goal=(1,1), start=None, warn=True, startStatao=None):
    

# Criado uma coisa aqui
...
def foodHeuristic(state, problem):
  ...
  # # grid de comidas e paredes
  # comidas_grid = state[1]

  # print "currGrid"
  # print comidas_grid

  # # transforma em uma lista de pontos (x,y)
  # comidas = comidas_grip.asList()

  # # numero muito pequeno para sobrepor com a distancia no loop
  # distanciaMaxima = -sys.maxint -1
  
  # # loop pelas comidas, procurando a com maior distancia para retornar
  # for comida in comidas:
  #   distanciaManh = abs(comida[0] - pacmanLocal[0]) + abs(comida[1] - pacmanLocal[1])
  #   if distanciaManh > distanciaMaxima:
  #     distanciaMaxima = distanciaManh
